...
* Important :
*
* 1. SparkFun_BMI270_Arduino_Library or Arduino_BMI270_BMM150 have not been used based on the assumption that we do not have SparkFun BMI270 6DoF IMU Breakout or Arduino Nano 33 BLE Sense Rev2 but rather only a standalone working BMI270 IMU. 
* The API provided directly by Bosh Sensor Tech BMI270_Sensor_API to connect to the BMI270 IMU has been used.
*
* 2. Due to lack of avability of BMI270 the code cannot be verified on actual hardware setup. 
* The working of sensor data processing part of code has been verified by making some fake sensor data and performing state calculations on it. This is available in the file check_sensor_data_processing.ino . 
* For the final file many examples have been referred to prevent against any errors. A BMI270 Arduino Interface Simulator also couldn't be found.
*
* 3. The accelerometer sensor has been assumed to be located at the center of mass of the system. 
* Thus accelerometer only measures linear acceleration and gyroscope readings can be used to get orientation.
*
* 4. Accelerometer values can be used along with gyroscope to give more accurate estimation of orientation. 
* However this only works when there isn't linear motion since we use gravity to calculate the angle change. 
* Enough linear acceleration has been assumed to make readings of accelerometer for orientation calculation incorrect.
* 
* 5. BMI270 accelerometer and gyroscope have the same coordinate axes as specified in the datasheet.
*/
